...
# ----- key coursework task parameters ----- #False
...
class Task1(Task):
  ...
  def spawn_task_objects(self):
    ...
    # # Amlan Custom Section
    # # Use fixed position
    # fixed_position = [0.5, 0.2]  # Replace with your desired coordinates
    # rot_lims = [0, 0]  # No rotation
    # self.spawn_model(name=random_object, point=fixed_position, rotationlims=rot_lims)
...
class Task2(Task):
  ...
  def spawn_task_objects(self):
    ...
    if T2_GROUND_PLANE_NOISE < 1e-3:
      ...
      tile_height = T2_GROUND_PLANE_NOISE
      self.reset_task(respawn_tiles=True, tile_height=tile_height)
